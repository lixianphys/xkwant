...
def test_doubledirac_mkhbar_4t(
    ...
    def hop_y(site1, site2):
        return -1j * vs / (2 * a) * np.kron(s_x, tau_z) - ws * np.kron(s_z, tau_0)

    def lead_hop_x(site1, site2):
        return +1j * vl / (2 * a) * np.kron(s_y, tau_z) - wl * np.kron(s_z, tau_0)

    def lead_hop_y(site1, site2):
        return -1j * vl / (2 * a) * np.kron(s_x, tau_z) - wl * np.kron(s_z, tau_0)
