...
def rashba_vary_lambda(
    ...
    energy_range = np.arange(
        0, 0.05, 0.0001
    )  # This determines the energy resolution in the integrated DOS. The higher the better but also more expensive.

    target_energies = [
        density_to_energy(
            *varyx_idos(
                mkhbar_4t, geop, hamp_sys, hamp_lead, "vs", xvalue, energy_range
            ),
            target_density,
        )
    ...
    target_energies_6t = [
        density_to_energy(
            *varyx_idos(
                mkhbar_6t, geop, hamp_sys, hamp_lead, "vs", xvalue, energy_range
            ),
            target_density,
        )
        for xvalue in (la / 1e3 for la in lamd)
    ...
    for xvalue, energy in zip((la / 1e3 for la in lamd), target_energies_6t):
        voltages_6t, Isup, Isdown = varyx_voltage_6t(
            mkhbar_6t,
            geop,
            hamp_sys,
            hamp_lead,
    ...
    for i, (xvalue, energy, voltages) in enumerate(
        ...
        rho_site, J_site = varyx_rho_j_energy_site(
            mkhbar_4t, geop, hamp_sys, hamp_lead, "vs", xvalue, energy
        )
        ...
        if rho_site is not None:
            ...
            print(
                f"At $\\lambda$={xvalue*1e3}, the number of modes:{total_modes}, energy is {energy}"
            )
            ...
            plt.savefig(os.path.join(savepath, f"rashba_lambda_{xvalue*1e3}_plot.png"))
...
def rashba_vary_density(
    ...
    # First calculate the integral density of state to further derive the target energy for later calculations which correspond to the fixed target density
    energy_range = np.arange(0, 0.015, 0.0001)
    target_energies = [
        density_to_energy(*get_idos(syst, energy_range), target_density)
        for target_density in densities
    ]
    ...
    for i, (energy, voltages) in enumerate(zip(target_energies, voltages_list)):
        ...
        if rho_site is not None:
            ...
            plt.savefig(
                os.path.join(
                    savepath, f"rashba_lamb_{lamb*1e3}_density_{densities[i]:0.5f}.png"
                )
            )
            plt.close(fig)  # to avoid 'figure.max_open_warning'
